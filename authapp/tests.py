...
from authapp.utils import generate_token


class BaseTest(TestCase):
    def setUp(self):
        self.register_url=reverse('authapp:register')
...
class UserVerifyTest(BaseTest):
    def test_user_ctivates_success(self):
        ...
        response=self.client.get(reverse('authapp:activate',kwargs={'uidb64':uid,'token':token}))
        self.assertEqual(response.status_code,302)
